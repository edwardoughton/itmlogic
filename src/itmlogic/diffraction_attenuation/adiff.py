...
def adiff(d, prop):
    ...
    if d == 0:
        ...
        prop["afo"] = min(
            15,
            2.171
            * np.log(1 + 4.77e-4 * prop["hg"][0] * prop["hg"][1] * prop["wn"] * q),
        )
        ...
        adiff1 = aknfe(q * prop["dl"][0] / (ds + prop["dl"][0])) + aknfe(
            q * prop["dl"][1] / (ds + prop["dl"][1])
        )
        ...
        q = (prop["wd1"] + prop["xd1"] / d) * min(
            ((1 - 0.8 * math.exp(-d / 50e3)) * prop["dh"] * prop["wn"]), 6283.2
        )
