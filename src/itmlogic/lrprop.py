...
def lprop (d, prop):
    ...
    if prop['mdp'] != 0:
        prop['dls'] = math.sqrt(2 * prop['he'] / prop['gme'])
    ...
    if abs(prop['the'][0]) > 0.2 or prop['dl'][0] < 0.1 * prop['dls'][0] or
        (prop['dl'][1] > 3 * prop.['dls'][0]) prop.kwx=max(prop.kwx,3) end
    
    if abs(prop['the'][1]) > 0.2 or prop['dl'][1] < 0.1 * prop['dls'][1]) or
