...
def lrprop(d, prop):
    ...
    if prop["mdp"] != 0:
        ...
        if (
            abs(prop["the"][0]) > 0.2
            or prop["dl"][0] < 0.1 * prop["dls"][0]
            or prop["dl"][1] > 3 * prop["dls"][0]
        ):
            prop["kwx"] = max(prop["kwx"], 3)

        if (
            abs(prop["the"][1]) > 0.2
            or prop["dl"][1] < 0.1 * prop["dls"][1]
            or prop["dl"][1] > 3 * prop["dls"][1]
        ):
            prop["kwx"] = max(prop["kwx"], 3)
        ...
        if (
            ...
            or prop["gme"] < 75e-9
            or prop["gme"] > 250e-9
            or prop["zgnd"].real < abs(prop["zgnd"].imag)
            or prop["wn"] < 0.419
            or prop["wn"] > 420
        ):
            prop["kwx"] = 4
        ...
        prop["wis"] = 0.021 / (
            0.021 + prop["wn"] * prop["dh"] / max(10e3, prop["dlsa"])
        )
        prop["ascat1"] = 0
        ...
        if prop["wscat"] == 0:
            ...
            if a5 < 1000:
                ...
                prop["dx"] = max(
                    [
                        prop["dlsa"],
                        prop["dla"] + 0.3 * prop["xae"] * np.log(47.7 * prop["wn"]),
                        (a5 - prop["aed"] - prop["ems"] * d5)
                        / (prop["emd"] - prop["ems"]),
                    ]
                )
